<?php
	 ...
	 */
function go_make_is_stripe_checkout_class( $classes ) {
	if ( function_exists( 'edd_get_option' ) && ! empty( edd_get_option( 'stripe_checkout' ) ) ) {
		$classes[] = 'edd-stripe-checkout';
	}
	return $classes;
}
	add_filter( 'body_class', 'go_make_is_stripe_checkout_class' );
	 ...
	 */
function go_make_edd_purchase_form_required_fields( $required_fields ) {
	unset( $required_fields['edd_first'] );
	unset( $required_fields['edd_last'] );
	return $required_fields;
}
	add_filter( 'edd_purchase_form_required_fields', 'go_make_edd_purchase_form_required_fields' );
	 ...
	 */
function go_make_edd_remove_names() {
	remove_action( 'edd_purchase_form_after_user_info', 'edd_user_info_fields' );
}
	add_action( 'init', 'go_make_edd_remove_names' );
	 ...
	 */
function go_make_edd_user_info_fields() {
	...
	add_action( 'edd_purchase_form_after_user_info', 'go_make_edd_user_info_fields' );
	 ...
	 */
function go_make_edd_force_single_item_cart() {
	 edd_empty_cart();
	return edd_get_cart_contents();
}
	// add_filter( 'edd_add_to_cart', 'go_make_edd_force_single_item_cart', 1, 1 );
	 ...
	 */
function go_make_edd_only_load_files_on_checkout() {
	if ( is_page( 'checkout' ) ) {
		return;
	}
	...
	add_action( 'wp_enqueue_scripts', 'go_make_edd_only_load_files_on_checkout' );
	 ...
	 */
function go_make_edd_remove_credit_card_validator() {
	wp_dequeue_script( 'creditCardValidator' );
}
	add_action( 'wp_enqueue_scripts', 'go_make_edd_remove_credit_card_validator' );
	 ...
	 */
function go_make_edd_no_js_disable_purchase() {
	...
	echo '<div id="go-make-edd-no-js-purchase-message">' .
			'<em>' . __( 'Please enabled JavaScript to complete your purchase.', 'go-make-things' ) . '</em><br>' .
			'<button class="btn btn-large disabled" disabled="disabled">' . $complete_purchase . '</button>' .
		'</div>';
}
	add_action( 'edd_purchase_form_after_submit', 'go_make_edd_no_js_disable_purchase' );
	 ...
	 */
function go_make_edd_checkout_submit() {
	?>
	...
	add_action( 'edd_purchase_form_after_cc_form', 'go_make_edd_checkout_submit', 9999 );

function go_make_edd_remove_checkout_submit() {
	 remove_action( 'edd_purchase_form_after_cc_form', 'edd_checkout_submit', 9999 );
}
	add_action( 'init', 'go_make_edd_remove_checkout_submit' );
	 ...
	 */
function go_make_edd_add_cart_link_to_nav( $items, $args ) {
	 ...
	 */
function go_make_edd_add_payment_icons( $icons ) {
	unset( $icons );
	$icons = array(
		'visa' => 'Visa',
	...
	add_filter( 'edd_accepted_payment_icons', 'go_make_edd_add_payment_icons' );
...
function go_make_edd_remove_show_payment_icons() {
	 ...
	 * @return void
	 */
function go_make_edd_show_payment_icons() {
	...
	add_action( 'edd_payment_mode_top', 'go_make_edd_show_payment_icons' );
	add_action( 'edd_checkout_form_top', 'go_make_edd_show_payment_icons' );
