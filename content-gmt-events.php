...
if ( ! empty( $details['video'] ) ) {
	$materials['Video'] = $details['video'];
}
if ( ! empty( $details['slides'] ) ) {
	$materials['Slides'] = $details['slides'];
}
if ( ! empty( $details['audio'] ) ) {
	$materials['Audio'] = $details['audio'];
}
if ( ! empty( $details['downloads'] ) ) {
	$materials['Downloads'] = $details['downloads'];
}
if ( ! empty( $details['post'] ) ) {
	$materials['Post'] = $details['post'];
}
...
if ( is_single() ) :
	...
	<?php if ( ! empty( $date ) || ! empty( $details['time_start_hour'] ) || ! empty( $details['location'] ) ) : ?>
			<p>
				...
				<?php
				if ( ! empty( $date ) && ( ! empty( $details['time_start_hour'] ) || ! empty( $details['location'] ) ) ) :
					?>
					<br>
					<?php
				endif;
				...
				<?php
				if ( ! empty( $details['time_start_hour'] ) && ! empty( $details['location'] ) ) :
					?>
					<br>
					<?php
				endif;
	...
	?>
		...
		<?php
		// Variables
		global $show_go_make_events_header_future, $show_go_make_events_header_past;
		$options = events_get_theme_options();
		?>

		<?php
		// Add header
		if ( $show_go_make_events_header_future && $start_date >= $today ) :
			$show_go_make_events_header_future = false;
			?>
		<h2><?php echo esc_html( $options['heading_future'] ); ?></h2>
			<?php
		elseif ( $show_go_make_events_header_past && $start_date < $today ) :
			$show_go_make_events_header_past = false;
			?>
	...
	<article class="margin-bottom">
		...
		<header>
			<h3 class="h5 no-padding-top no-margin-bottom">
				...
				<?php
					// Add link to edit pages
					edit_post_link( __( 'Edit', 'go-make-things' ), '&ndash; ', '' );
				?>
			</h3>
		</header>
