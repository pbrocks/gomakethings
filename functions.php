 ...
 * Load theme files
 */
function go_make_load_theme_files() {
	$go_make_theme = wp_get_theme();
	wp_enqueue_style( 'go-make-theme-styles', get_template_directory_uri() . '/dist/css/main.min.' . $go_make_theme->get( 'Version' ) . '.css', null, null, 'all' );
	if ( isset( $_COOKIE['fontsLoaded'] ) && $_COOKIE['fontsLoaded'] === 'true' ) {
		wp_enqueue_style( 'go-make-theme-fonts', 'https://fonts.googleapis.com/css?family=PT+Serif:400,400i,700', null, null, 'all' );
	}
}
add_action( 'wp_enqueue_scripts', 'go_make_load_theme_files' );

/**
 * Load inline header content
 */
function go_make_load_inline_header() {
								  $go_make_theme = wp_get_theme();
	?>
			<script>
			<?php echo file_get_contents( get_template_directory_uri() . '/dist/js/detects.min.' . $go_make_theme->get( 'Version' ) . '.js' ); ?>
...
add_action( 'wp_head', 'go_make_load_inline_header', 30 );

/**
 * Load inline footer content
 */
function go_make_load_inline_footer() {
								  global $post;
	$go_make_theme = wp_get_theme();
	...
	<?php echo file_get_contents( get_template_directory_uri() . '/dist/js/loadJS.min.' . $go_make_theme->get( 'Version' ) . '.js' ); ?>
		if ( 'querySelector' in document && 'addEventListener' in window ) {
			loadJS('<?php echo get_template_directory_uri() . '/dist/js/main.min.' . $go_make_theme->get( 'Version' ) . '.js'; ?>');
		<?php if ( ! empty( $get_checkout ) && is_page( $get_checkout ) && empty( edd_get_option( 'stripe_checkout' ) ) ) : ?>
				loadJS('<?php echo get_template_directory_uri() . '/dist/js/cleave.min.' . $go_make_theme->get( 'Version' ) . '.js'; ?>');
		...
		<?php endif; ?>
			if (document.querySelectorAll('#edd_purchase_form, .mailchimp-form, .edd-self-service-form').length > 0) {
				loadJS('<?php echo get_template_directory_uri() . '/dist/js/validate.min.' . $go_make_theme->get( 'Version' ) . '.js'; ?>');
			}
		}
	</script>
	<?php
}
add_action( 'wp_footer', 'go_make_load_inline_footer', 30 );

function go_make_header_classes() {
								  $classes = '';
	if ( isset( $_COOKIE['fontsLoaded'] ) && $_COOKIE['fontsLoaded'] === 'true' ) {
 ...
 */
function go_make_wpsearch() {
	return get_search_form();
}
add_shortcode( 'searchform', 'go_make_wpsearch' );
 ...
 */
function go_make_get_theme_directory_uri() {
	return get_template_directory_uri();
}
add_shortcode( 'themeuri', 'go_make_get_theme_directory_uri' );
 ...
 */
function go_make_get_site_url() {
	return site_url();
}
add_shortcode( 'siteurl', 'go_make_get_site_url' );
 ...
 */
function go_make_custom_rss_feed( $output, $feed ) {
	if ( strpos( $output, 'comments' ) ) {
		return $output;
	}
	return esc_url( 'http://feeds.feedburner.com/GoMakeThings' );
}
add_action( 'feed_link', 'go_make_custom_rss_feed', 10, 2 );
 ...
 */
function go_make_post_password_form() {
								  global $post;
	$label = 'pwbox-' . ( empty( $post->ID ) ? rand() : $post->ID );
	$form =
		'<form class="text-center" action="' . esc_url( site_url( 'wp-login.php?action=postpass', 'login_post' ) ) . '" method="post"><p>' . __( 'This is a password protected post.', 'go-make-things' ) . '</p><label class="screen-reader" for="' . $label . '">' . __( 'Password', 'go-make-things' ) . '</label><input id="' . $label . '" name="post_password" type="password"><input type="submit" name="Submit" value="' . __( 'Submit', 'go-make-things' ) . '"></form>';
	return $form;
}
add_filter( 'the_password_form', 'go_make_post_password_form' );
 ...
 */
function go_make_pretty_wp_title( $title, $sep ) {
	...
	if ( $paged >= 2 || $page >= 2 ) {
		$title = "$title $sep " . sprintf( __( 'Page %s', 'go-make-things' ), max( $paged, $page ) );
	}

	return $title;
}
add_filter( 'wp_title', 'go_make_pretty_wp_title', 10, 2 );
 ...
 */
function go_make_excerpt_length( $length ) {
	return 35;
}
add_filter( 'excerpt_length', 'go_make_excerpt_length', 999 );
 ...
 */
function go_make_excerpt_more( $more ) {
	// return '... <a href="'. get_permalink( get_the_ID() ) . '">' . __('Read More', 'go-make-things') . '</a>';
	return '...';
}
add_filter( 'excerpt_more', 'go_make_excerpt_more' );
 ...
 */
function go_make_register_menus() {
	register_nav_menus(
		array(
...
add_action( 'init', 'go_make_register_menus' );
 ...
 */
function go_make_post_thumbnails_support() {
	add_theme_support( 'post-thumbnails', array( 'gmt-projects', 'page', 'download' ) );
}
add_action( 'after_setup_theme', 'go_make_post_thumbnails_support' );
 ...
 */
function go_make_comment_layout( $comment, $args, $depth ) {
	...
	?>
	 id="comment-<?php comment_ID(); ?>">
		...
		<?php if ( $comment->comment_approved == '0' ) : // If the comment is held for moderation ?>
				<p><em><?php _e( 'Your comment is being held for moderation.', 'go-make-things' ); ?></em></p>
		<?php endif; ?>
 ...
 */
function go_make_dequeue_devicepx() {
	wp_dequeue_script( 'devicepx' );
}
add_action( 'wp_enqueue_scripts', 'go_make_dequeue_devicepx', 20 );
...
function go_make_remove_all_jetpack_css() {
...
add_action( 'wp_print_styles', 'go_make_remove_all_jetpack_css' );
 ...
 */
function go_make_process_jetpack_markdown( $content ) {
 ...
 */
function go_make_get_jetpack_markdown( $options, $name, $suffix = '_markdown' ) {
 ...
 */
function go_make_allow_new_html_tags() {
	global $allowedposttags;
	$allowedposttags['markdown'] = true;
}
add_action( 'init', 'go_make_allow_new_html_tags' );

/**
 * Disable WordPress auto-formatting
 * Disabled by default. Uncomment add_action to enable
 */
function go_make_remove_wpautop() {
						remove_filter( 'the_content', 'wpautop' );
}
// add_action( 'pre_get_posts', 'go_make_remove_wpautop' );
 ...
 */
function go_make_remove_empty_p( $content ) {
	$content = force_balance_tags( $content );
	$content = preg_replace( '#<p>\s*+(<br\s*/*>)?\s*</p>#i', '', $content );
	$content = preg_replace( '~\s?<p>(\s|&nbsp;)+</p>\s?~', '', $content );
	return $content;
}
add_filter( 'the_content', 'go_make_remove_empty_p', 20, 1 );
 ...
 */
function go_make_update_image_default_link_type() {
					  update_option( 'image_default_link_type', 'none' );
}
add_action( 'admin_init', 'go_make_update_image_default_link_type' );
 ...
 */
function go_make_modify_projects_query( $query ) {

	if ( is_admin() || ! is_post_type_archive( 'gmt-projects' ) || $query->get( 'post_type' ) !== 'gmt-projects' ) {
		return;
	}
...
add_action( 'pre_get_posts', 'go_make_modify_projects_query' );
 ...
 * @param  array $query The WordPress post query
 */
function go_make_modify_events_query( $query ) {

	if ( is_admin() || ! is_post_type_archive( 'gmt-events' ) || $query->get( 'post_type' ) !== 'gmt-events' ) {
		return;
	}
...
add_action( 'pre_get_posts', 'go_make_modify_events_query' );
 ...
 */
function go_make_reorder_events( $wp_query ) {
 ...
 */
function go_make_redirect_single_events() {
...
add_action( 'template_redirect', 'go_make_redirect_single_events' );
 ...
 */
function go_make_just_comments_count() {
	global $post;
	return count(
		get_comments(
			array(
 ...
 */
function go_make_trackbacks_count() {
	global $post;
	return count(
		get_comments(
			array(
 ...
 */
function go_make_is_last_post( $wp_query ) {
 ...
 * @link https://github.com/viewportindustries/starkers/
 */
function go_make_get_page_id_from_path( $path ) {
